...
const int kGraphImageWidth       = 640;  // The width of the pixels going into the graph.
const int kGraphImageHeight      = 480;  // the height of the pixels going into the graph.
const float kPredictionThreshold = 0.50; // Prediction percentages lower than this will be discarded.
const int kGraphMaxPredictions   = 5;   // After this many predictions we move on.
...
- (id) init
    {
        ...
        if (self)
        {
            ...
            session.clientId = @"d:yourorg-id:camera:ios";
            session.userName = @"use-token-auth";
            session.password = @"your-authentication-token@";
            [session connectToHost:@"org-id.messaging.internetofthings.ibmcloud.com" port:1883 usingSSL:NO];
            */
            // NH: MQTT - end
        }
        return self;
            ...
            if (tfSession.get())
            {
                ...
                else
                {
                    ...
                    for (int i = 0; i < kGraphMaxPredictions; ++i)
                    {
                        ...
                        NSString *phoneLabel = @"phone";
                        if ([prediction.label isEqualToString:phoneLabel])
                        {
                            printf("Phone detected %i\n", self.framesProcessed);
                            // NH: MQTT - start
                            /*
                            [session publishData:[@"{}" dataUsingEncoding:NSUTF8StringEncoding]
                                onTopic:@"iot-2/evt/phone-discovered/fmt/json"
                                retain:NO
